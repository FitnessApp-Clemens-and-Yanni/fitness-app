...
export function NutritionalValuesProgressBars(props: {
    ...
    const currentValue = props.dailyNutritionalData[props.nutrientKey];
    ...
    return (
        <View className="w-full">
            <View className={props.viewTextClasses}>
                <Text>
                    {props.nutritionalValue} {currentValue.toFixed(1)}{" / "}{targetValue}
                </Text>
            </View>
            <Progress
                value={progressValue}
                className={props.progressClasses}
            />
        </View>
    )
}